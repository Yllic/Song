...
const Search = () => {
  ...
  useEffect(() => {
    searchHotDetail()
      .then((res) => (
        setDatail(res.data.data.map((item) => item))
      ))
      .catch((err) => (
        console.log(err)
      ))
  ...
  const icon = ["octicon:person-fill-16", "clarity:book-solid", "game-icons:musical-notes", "ph:microphone-fill"];
  // 跳转至首页
    ...
    <div style={{backgroundColor: "#f4f4f4"}}>
      <StyledDiv>
        <div className="search">
          ...
          <div>搜索</div>
          <Icon icon="tabler:search" color="#6f778f" width="16" style={{position: "absolute", top: "8vw", left: "13.68vw"}} />
        </div>
        ...
        <div className="search_like">
          <div className="search_like__title">
            <h4>猜你喜欢</h4>
            <Icon icon="mdi:refresh" color="#acafae" style={{width: "5vw", height: "5vw"}} onClick={handleRefreshClick} />
          </div>
